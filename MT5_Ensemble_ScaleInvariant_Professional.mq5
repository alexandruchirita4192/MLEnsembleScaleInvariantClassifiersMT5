...
void LogInfo(string message) {
  if (InpLog) Print(message);
}

void LogDebug(string message) {
  if (InpLog && InpDebugLog) Print(message);
}
...
bool NormalizeWeights() {
  ...
  if (s <= 0.0) {
    LogInfo(
        ...
        "InpMlpWeight/InpLgbmWeight/InpHgbWeight.");
    return false;
  }
...
bool ParseHoursCsv(const string csv, bool &hours_mask[], bool &has_any_values,
  ...
  if (StringLen(normalized) == 0) return true;
...
bool IsHourAllowedByMasks(int hour_value) {
  if (hour_value < 0 || hour_value > 23) return false;

  if (g_has_allowed_hours_csv && !g_allowed_hours_mask[hour_value])
    return false;

  if (g_denied_hours_mask[hour_value]) return false;
...
double ComputeLotSize(const SignalInfo &info) {
  ...
  if (!InpUseConfidenceSizing) return NormalizeVolumeToSymbol(lots);
...
void RefreshClosedDealState() {
  ...
  for (int i = g_last_history_deals_total; i < total; i++) {
    ...
    if (deal_day_key == g_guard_day_key) g_guard_day_closed_pnl += net;
  ...
  if (InpUseDailyLossGuard && !g_daily_loss_guard_active &&
    ...
    if (InpDailyLossFlatOnTrigger) CloseOpenPosition();
  }
}
...
int OnInit() {
  ...
  LogInfo(StringFormat(
      "OnInit complete. weights=(%.3f, %.3f, %.3f) historyDeals=%d", g_w_mlp,
      g_w_lgbm, g_w_hgb, g_last_history_deals_total));
...
void OnTick() {
  ...
  if (!EntryGuardsAllow(atr14_raw)) return;
